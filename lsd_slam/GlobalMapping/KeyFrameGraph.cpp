...
// (dirty?) fix for undefined references
#define cs_di_sparse cs_sparse
#define cs_di_symbolic cs_symbolic
...
void KeyFrameGraph::dumpMap(std::string folder)
{
	...
	for (unsigned int i = 0; i < keyframesAll.size(); ++i) {
		...
		// write camToWorld
		fle 	<< "KF#" << i << ","
			<< kf->getScaledCamToWorld().data()[0] << ","
			<< kf->getScaledCamToWorld().data()[1] << ","
			<< kf->getScaledCamToWorld().data()[2] << ","
			<< kf->getScaledCamToWorld().data()[3] << ","
			<< kf->getScaledCamToWorld().data()[4] << ","
			<< kf->getScaledCamToWorld().data()[5] << ","
			<< kf->getScaledCamToWorld().data()[6] << ","
