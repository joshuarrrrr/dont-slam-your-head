...
: SLAMEnabled(enableSLAM), relocalizer(w,h,K)
{
	if(w%16 != 0 || h%16!=0)
	{
		LOGD("image dimensions must be multiples of 16! Please crop your images / video accordingly.\n");
		assert(false);
	}
...
SlamSystem::~SlamSystem()
{
	...
	LOGD("... waiting for SlamSystem's threads to exit\n");
	...
	LOGD("DONE waiting for SlamSystem's threads to exit\n");
...
void SlamSystem::mappingThreadLoop()
{
	...
	LOGD("Exited mapping thread \n");
}

void SlamSystem::finalize()
{
	LOGD("Finalizing Graph... finding final constraints!!\n");
	...
	LOGD("Finalizing Graph... optimizing!!\n");
	...
	LOGD("Finalizing Graph... publishing!!\n");
	...
	usleep(200000);
	LOGD("Done Finalizing Graph.!!\n");
}


void SlamSystem::constraintSearchThreadLoop()
{
	LOGD("Started  constraint search thread!\n");
	...
	while(keepRunning)
	{
		if(newKeyFrames.size() == 0)
		{
			...
			if(!doneSomething)
			{
				if(enablePrintDebugInfo && printConstraintSearchInfo)
					LOGD("nothing to re-track... waiting.\n");
		...
		if(doFullReConstraintTrack)
		{
			...
			LOGD("Done optizing Full Map! Added %d constraints.\n", added);
	...
	LOGD("Exited constraint search thread \n");
}

void SlamSystem::optimizationThreadLoop()
{
	LOGD("Started optimization thread \n");
	...
	while(keepRunning)
	{
		...
		{
			LOGD("doing final optimization iteration!\n");
	...
	LOGD("Exited optimization thread \n");
}
...
{
	if(enablePrintDebugInfo && printThreadingInfo)
		LOGD("DISCARDING KF %d\n", currentKeyFrame->id());

	if(currentKeyFrame->idxInKeyframes >= 0)
	{
		LOGD("WARNING: trying to discard a KF that has already been added to the graph... finalizing instead.\n");
	...
	else
	{
		if(force)
		{
			if(noCreate)
			{
				...
				nextRelocIdx = -1;
				LOGD("mapping is disabled & moved outside of known map. Starting Relocalizer!\n");
			}
			else
...
void SlamSystem::addTimingSamples()
{
	...
	if(sPassed > 1.0f)
	{
		...
		{
			LOGD("MapIt: %3.1fms (%.1fHz); Track: %3.1fms (%.1fHz); Create: %3.1fms (%.1fHz); FindRef: %3.1fms (%.1fHz); PermaTrk: %3.1fms (%.1fHz); Opt: %3.1fms (%.1fHz); FindConst: %3.1fms (%.1fHz);\n",
...
void SlamSystem::getDepthMap(float* depthMap, float* depthVar) {
	for (int y = 0; y < height; ++y)
		for (int x = 0; x < width; ++x) {
			int idx = x + y * width;
				depthMap[idx] = map->getDepthValue(x, y);
				depthVar[idx] = map->getDepthVariance(x, y);
		}
}
...
void SlamSystem::takeRelocalizeResult()
{
	...
	if(!tracker->trackingWasGood || tracker->lastGoodCount / (tracker->lastGoodCount + tracker->lastBadCount) < 1-0.75f*(1-MIN_GOODPERGOODBAD_PIXEL))
	{
		if(enablePrintDebugInfo && printRelocalizationInfo)
			LOGD("RELOCALIZATION FAILED BADLY! discarding result.\n");
...
bool SlamSystem::doMappingIteration()
{
	...
	if(!doMapping && currentKeyFrame->idxInKeyframes < 0)
	{
		...
		map->invalidate();
		LOGD("Finished KF %d as Mapping got disabled!\n",currentKeyFrame->id());
	...
	if(trackingIsGood)
	{
		if(!doMapping)
		{
			//LOGD("tryToChange refframe, lastScore %f!\n", lastTrackingClosenessScore);
...
void SlamSystem::gtDepthInit(uchar* image, float* depth, double timeStamp, int id)
{
	...
	LOGD("Done GT initialization!\n");
}


void SlamSystem::randomInit(uchar* image, double timeStamp, int id)
{
	LOGD("Doing Random initialization!\n");

	if(!doMapping)
		LOGD("WARNING: mapping is disabled, but we just initialized... THIS WILL NOT WORK! Set doMapping to true.\n");
...
void SlamSystem::trackFrame(uchar* image, unsigned int frameID, bool blockUntilMapped, double timestamp)
{
	...
	{
		LOGD("TRACKING LOST for frame %d (%1.2f%% good Points, which is %1.2f%% of available points, %s)!\n",
				trackingNewFrame->id(),
	...
	if(blockUntilMapped && trackingIsGood)
	{
		...
		while(unmappedTrackedFrames.size() > 0)
		{
			//LOGD("TRACKING IS BLOCKING, waiting for %d frames to finish mapping.\n", (int)unmappedTrackedFrames.size());
			newFrameMappedSignal.wait(lock);
		}
		lock.unlock();
	}
		...
		float strictness)
{
	...
	{
		if(enablePrintDebugInfo && printConstraintSearchInfo)
			LOGD("FAILE %d -> %d (lvl %d): errs (%.1f / %.1f / -).",
				newKFTrackingReference->frameID, candidateTrackingReference->frameID,
				2,
	...
	{
		if(enablePrintDebugInfo && printConstraintSearchInfo)
			LOGD("FAILE %d -> %d (lvl %d): errs (%.1f / %.1f / %.1f).",
					newKFTrackingReference->frameID, candidateTrackingReference->frameID,
					1,
...
int SlamSystem::findConstraintsForNewKeyFrames(Frame* newKeyFrame, bool forceParent, bool useFABMAP, float closeCandidatesTH)
{
	...
	for(std::unordered_set<Frame*>::iterator c = candidates.begin(); c != candidates.end();)
	{
		...
		{
			if(enablePrintDebugInfo && printConstraintSearchInfo)
				LOGD("SKIPPING %d on %d cause it already exists as constraint.\n", (*c)->id(), newKeyFrame->id());
	...
	for(std::unordered_set<Frame*>::iterator c = closeCandidates.begin(); c != closeCandidates.end();)
	{
		...
		{
			if(enablePrintDebugInfo && printConstraintSearchInfo)
				LOGD("SKIPPING %d on %d (NEAR), cause we already have tried it.\n", (*c)->id(), newKeyFrame->id());
	...
	for(unsigned int i=0;i<farCandidates.size();i++)
	{
		...
		{
			if(enablePrintDebugInfo && printConstraintSearchInfo)
				LOGD("SKIPPING %d on %d (FAR), cause we already have tried it.\n", farCandidates[i]->id(), newKeyFrame->id());
	...
	if (enablePrintDebugInfo && printConstraintSearchInfo)
		LOGD("Final Loop-Closure Candidates: %d / %d close (%d failed, %d inconsistent) + %d / %d far (%d failed, %d inconsistent) = %d\n",
	...
	for (Frame* candidate : closeCandidates)
	{
		...
		if(e1 != 0)
		{
			...
			for(unsigned int k=0;k<farCandidates.size();k++)
			{
				...
				{
					if(enablePrintDebugInfo && printConstraintSearchInfo)
						LOGD(" DELETED %d from far, as close was successful!\n", candidate->id());
	...
	if(parent != 0 && forceParent)
	{
		...
		else
		{
			...
			LOGD("warning: reciprocal tracking on new frame failed badly, added odometry edge (Hacky).\n");
...
bool SlamSystem::optimizationIteration(int itsPerTry, float minChange)
{
	...
	if(enablePrintDebugInfo && printOptimizationInfo)
		LOGD("did %d optimization iterations. Max Pose Parameter Change: %f; avgChange: %f. %s\n", its, maxChange, sumChange / sum,
